// Generate a signed upload URL for artwork in Supabase Storage
export default async function handler(req, res) {
    ...
    const filename = body.filename || `artwork_${Date.now()}.jpg`;
    const safeName = filename.replace(/[^a-zA-Z0-9._-]/g, '_');
    const storagePath = `artworks/${Date.now()}_${safeName}`;
    ...
    try {
        ...
        console.error('Artwork upload URL error:', error.message);
